...
import * as models from "../../lib/GraphModels.ts";
...
export async function run({
 ...
 //throw new Error("hello.  is anyone there");
    ...
    if (!response.ok) {
        throw new Error(
            `Error Authenticating! status: ${response.status} : ${response.statusText} : ${fetchUrl} : ${formData}`,
        );
    }

    var authResponse = (await response.json()) as models.AuthResponse;
    ...
    const appResponse = await fetch(
        "https://graph.microsoft.com/v1.0/applications/44dc6d96-28ad-4a74-8f67-4397c2652eab",
        {
            method: "GET",
            headers: {
    ...
    if (!appResponse.ok) {
        throw new Error(
            `Error Getting App Info! status: ${appResponse.status} : ${appResponse.statusText}`,
        );
    }

    var appReg = (await response.json()) as models.AppRegistration;
