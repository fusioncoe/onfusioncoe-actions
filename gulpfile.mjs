...
async function nugetInstall(nugetSource, packageName, version, targetDir) {
  ...
  const feeds = {
    ...
    CAP_ISVExp_Tools_Stable: {
      ...
      // https://dev.azure.com/msazure/One/_packaging?_a=feed&feed=CAP_ISVExp_Tools_Daily
      baseUrl:
        "https://pkgs.dev.azure.com/msazure/_packaging/b0441cf8-0bc8-4fad-b126-841a6184e784/nuget/v3/flat2/",
    },
    "DPX-Tools-Upstream": {
      authenticated: true,
      // https://dev.azure.com/dynamicscrm/OneCRM/_artifacts/feed/DPX-Tools-Upstream
      baseUrl:
        "https://pkgs.dev.azure.com/dynamicscrm/OneCRM/_packaging/1b55351d-c15e-45b3-9454-70a9549af804/nuget/v3/flat2/",
    },
    PowerPortalPackages: {
      authenticated: true,
      // https://dynamicscrm.visualstudio.com/DefaultCollection/OneCRM/_packaging?_a=feed&feed=PowerPortalPackages
      baseUrl:
        "https://pkgs.dev.azure.com/dynamicscrm/b276c3e1-2902-46bd-a686-484157b97f48/_packaging/63f896c0-b555-4017-a932-9063f2e7487e/nuget/v3/flat2/",
    },
  };
  ...
  if (selectedFeed.authenticated) {
    if (!feedPAT) {
      throw new Error(
        `nuget feed ${nugetSource} requires authN but neither '--feedPAT' argument nor env var 'AZ_DevOps_Read_PAT' was defined!`,
      );
    }
    reqInit.headers["Authorization"] =
      `Basic ${Buffer.from("PAT:" + feedPAT).toString("base64")}`;
  }
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Cannot download ${res.url}, status: ${res.statusText} (${res.status}), body: ${body ? body.toString("ascii") : "null"}`,
    );
  }
...
function lint() {
  return gulp.src("src/**/*.ts").pipe(eslint()).pipe(eslint.format());
}

function test() 
{
    return gulp.src('tests') // Adjust the path to your test directory
        .pipe(jest({
        "preset": "ts-jest", // Use this if you're working with TypeScript
...
async function dist() {
  ...
  glob
    ...
    .map((actionName, idx) => {
      ...
      log.info(
        `package action ${idx} "${actionName}" into ./dist folder (${actionDistDir})...`,
      );
      esbuild.buildSync({
        bundle: true,
    ...
    fs.createReadStream(downloadedZipFile)
      ...
      .on("entry", function (entry) {
        ...
        if (filePath === `${packageNameToImport}.zip`) {
          entry
            .pipe(unzip.Extract({ path: portalPackageOutDir }))
            .on("close", () => {
              log.info(`Extracted portal package to ${portalPackageOutDir} `);
              fs.rmdirSync(path.resolve(outdir, "temp_portal_package"), {
                recursive: true,
              });
              resolve();
            })
...
const recompile = gulp.series(restore, compile);

const ci = gulp.series(restore, compile, lint, test);

const updateDist = gulp.series(restore, compile, dist, addDistToIndex);
